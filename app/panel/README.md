...
| `setup.js` | puesta en marcha dentro del panel: vincular WhatsApp (número → código VERIFICAR → sondeo), el paso 3 **Meta Ads** (`metaStep`: conectar o «Saltar por ahora», recordado en `localStorage` `comando.metaStepSkipped`; `metaStepPending` lo enseña solo sin Meta y sin CRM) y conectar el CRM desde Cuenta (HubSpot/Salesforce por Nango, Google Sheets con el selector de Google, desconectar, recuperar, purgar). Antes era el onboarding de `/app/`; ahora `/app/` solo es el acceso con Clerk. Mock del paso 3: `?mock=1&crm=sin&mk=sin` |
